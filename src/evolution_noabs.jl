...
function finite_differenced_pde_with_bc!(du, u, pHyper, t)
    ...
    Threads.@threads for n = NboundaryPadding + 2:Nx + NboundaryPadding
        ...
        dy1dt[n] = (
            ...
            - mphi2 * y0[n] 
            # interaction terms
            - mV * lambdaCross * y0[n]^(mV-1) * y2[n]^nV
            - lV * lambdaSelf * y0[n]^(lV-1)
        )
        ...
        dy3dt[n] = (
            ...
            - mchi2 * y2[n] 
            # interaction terms
            - nV * sigma * lambdaCross * y2[n]^(nV-1) * y0[n]^mV
            - lV * lambdaSelf * y2[n]^(lV-1)
        )
    end
    ...
    dy1dt[boundary_idx] = (
        ...
        # interaction terms
        - mV * lambdaCross * y0[boundary_idx]^(mV-1) * y2[boundary_idx]^nV
        - lV * lambdaSelf * y0[boundary_idx]^(lV-1)
    )      
    # no regularisation necessary
    dy2dt[boundary_idx] = (
        # definition
...
function check_field_too_large(u, t, integrator)
    max_value_allowed = 10^8  # Define your threshold here
    return any(x -> x > max_value_allowed, u[1:2:end])  # True if any field exceeds threshold
end
